...
点在透明处**不做任何反馈** —— 那里以前会进入「让位」态（立绘变半透明、
窗口暂时不接收触摸），那套机制已经整个去掉。
...
| 方案 | 结果 |
| :--- | :--- |
| `LayoutParams.setTouchableRegion`（只在不规则区域接收触摸） | 公开 SDK 里**不存在**，是 `@hide` 内部 API。已在 `android-36` 的 `android.jar` 里逐字节确认：`WindowManager$LayoutParams.class` 只有 `areWallpaperTouchEventsEnabled`、`setFrameRateBoostOnTouchEnabled` 这类无关方法 |
| 用多个矩形拼接近似人物轮廓 | 实测最多只能腾出 **18%** 面积。人物在画布上几乎铺满 —— 顶部只占 22% 宽，到 70% 高度处已占 83%，底部整行不透明 |
| 手势中途切换 `FLAG_NOT_TOUCHABLE` | 系统不会重新派发，那一下只会消失 |
| 按住立绘 + 另一根手指点桌面 | 技术上可行（曾实现过），但属于非常规操作，用户得先被告知才会用；容易误触，还可能出现窗口卡在「点不动」的状态，需要额外定时器兜底 |
| 把透明部分裁掉 | 窗口只能是矩形，裁掉透明区就会切到人物本体（左右各裁 8% 会切掉 7.8% 的人物像素，主要是发丝与裙摆） |

**去掉之后的好处**：立绘不会突然变半透明，不会出现「点不动」的状态，
也少了一条跨 JS/原生的状态链路（闪烁与时序问题的来源之一）。
