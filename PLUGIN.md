...
点立绘的**透明区域**应该点到桌面图标，点**人物本体**应该收放对话框。
两者用同一个判定入口，避免两套逻辑互相抢先。

### 判定（网页侧，只有两步）

1. **`target.closest(CLICK_THROUGH_KEEP)`** —— 命中对话框、输入框、发送、
   ＋菜单、配置页等交互控件就正常处理，绝不穿透。
2. **`portraitAlphaAt(x, y)`** —— 立绘 PNG 自带透明通道，用 1×1 的 canvas 取样。
   要注意 `object-fit: contain` + `object-position: bottom center` 的换算，
   否则取样坐标会偏（实测四角 alpha=0、中心 255，换算正确）。

alpha 够高 → 人物本体 → 收放对话框；alpha 接近 0 → 透明处 → 进入穿透态。

### 穿透为什么需要「持续一小段时间」

**这里有个很容易想错的地方，也是这个功能长期看着像好的、实际没生效的原因。**

`FLAG_NOT_TOUCHABLE` 决定整窗是否接收触摸。原设计的流程是：

```
轻点透明处 → setTouchable(false) → 「这一下落到桌面」→ 140ms 后恢复
```

但 Android 是在**手势开始时**读一次这个标志来决定事件归谁，
中途改它**不会**把已经开始的这一下重新派发给下层窗口 ——
它只是让当前这一下从此收不到后续事件，然后消失。
所以原设计的实际效果是「点透明处毫无反应」，而不是穿透。

现在的做法是让穿透态**持续 1300ms**：

```
第一次轻点透明处 → 进入穿透态（这一下被吃掉，本来也到不了桌面）
紧接着再点一下   → 这一次真正落到底下的桌面图标
1300ms 后       → 自动恢复可触摸
```

代价是穿透需要点两下（第一下「解锁」），但它是真的能用。
另外每 2 秒有一次兜底 `setTouchable(true)`，保证窗口不会永久卡在点不动。

### 两个必须注意的坑

**恢复触摸的定时器一定要能独立兜底。** 穿透期间网页收不到任何事件，
所以恢复只依赖 `setTimeout`，不依赖后续事件。

**判断「是不是轻点」不要依赖 `holdTimer` 是否非零。**
`clearHold()` 会把它清成 0，任何一次多余的 `clearHold` 都会让轻点被误判成
「不是轻点」，表现就是「点立绘没反应」。用显式的 `longPressed` 标记才对 ——
这个 bug 实际发生过，轻点收放气泡因此完全失效。

**不要累积 `pointermove` 的最大位移来判断拖动。** 指针拖远再拖回来时那个
历史值仍然很大，会把之后的轻点全部误判成拖动。只比较按下与抬起的位移即可。
